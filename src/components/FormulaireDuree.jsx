...
const FormulaireDuree = () => {
  const { neuf, mensualite, taux, assurance, montant } = useSelector(state => state);
  const dispatch = useDispatch();

  const handleButton = useCallback(() => {
    dispatch(updateFormulaire({ neuf: !neuf }));
  }, [dispatch, neuf]);

  const handleValue = useCallback((champ, value) => {
    dispatch(updateFormulaire({ [champ]: value }));
  }, [dispatch]);
  ...
  return (
    <Col xs="12" md="5" className="px-5">
      ...
      <Label className="mb-4 mt-3 label">Montant du prêt</Label>
      <InputRange
        step={5000} formatLabel={v => `${v}€`} minValue={0} maxValue={500000}
        value={montant} onChange={v => handleValue('montant', v)} />

      <Label className="mb-4 mt-3 label">Mensualité souhaitée</Label>
      <InputRange
        step={50} formatLabel={v => `${v}€`} minValue={50} maxValue={10000}
...
export default FormulaireDuree;
